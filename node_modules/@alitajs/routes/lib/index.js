"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = _default;

function _assert() {
  const data = _interopRequireDefault(require("assert"));

  _assert = function _assert() {
    return data;
  };

  return data;
}

var _exclude = _interopRequireDefault(require("./exclude"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function optsToArray(item) {
  if (!item) return [];

  if (Array.isArray(item)) {
    return item;
  } else {
    return [item];
  }
}

function _default(api) {
  // disable if routes if configured
  if (api.userConfig.routes) return;
  api.describe({
    key: 'routesExtend',
    config: {
      schema(joi) {
        return joi.object();
      }

    }
  });
  api.modifyRoutes(routes => {
    const routesExtend = api.config.routesExtend;
    if (!routesExtend) return routes;
    routes = (0, _exclude.default)(routes, optsToArray(routesExtend.exclude));

    if (routesExtend.update) {
      (0, _assert().default)(typeof routesExtend.update === 'function', `opts.update should be function, but got ${routesExtend.update}`);
      routes = routesExtend.update(routes);
    }

    return routes;
  });
}