...
exports.normalizeReqPath = exports.getMfsuPath = exports.checkConfig = void 0;
...
const normalizeReqPath = (api, reqPath) => {
  ...
  const isMfAssets = reqPath.startsWith(`${normalPublicPath}mf-va_`) || reqPath.startsWith(`${normalPublicPath}mf-dep_`) || reqPath.startsWith(`${normalPublicPath}mf-static/`);
  ...
  api.onPluginReady({
    fn() {
      ...
      if (['dev', 'build'].includes(command)) {
        console.log(_utils().chalk.hex('#faac00')('MFSU Enabled'));
      }
    },
    ...
    return (req, res, next) => {
      ...
      if (isMfAssets) {
        depBuilder.onBuildComplete(() => {
          ...
          const content = (0, _fs().readFileSync)((0, _path().join)(mfsuPath, fileRelativePath), 'utf-8');
          res.setHeader('content-type', _mime().default.lookup((0, _path().parse)(path || '').ext)); // 排除入口文件，因为 hash 是入口文件控制的
